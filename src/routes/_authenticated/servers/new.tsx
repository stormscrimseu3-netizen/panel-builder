...
function NewServerPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl p-6 md:p-10">
      ...
      <p className="text-sm text-muted-foreground">Choose an egg, then fill the startup variables.</p>
      ...
      <form onSubmit={submit} className="card-elevated mt-6 space-y-5 rounded-2xl p-6">
        <div className="grid gap-4 md:grid-cols-[1fr_1.1fr]">
          ...
          <div className="space-y-2">
            ...
            <Select value={eggId} onValueChange={selectEgg}>
              ...
              <SelectContent>
                {eggs.map((egg) => (
                  <SelectItem key={egg.id} value={egg.id}>{egg.category} / {egg.name}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge variant="outline" className="mono">{selectedEgg.image}</Badge>
              </div>
        ...
        <div className="space-y-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Required, secret, and validation rules match the selected egg.</p>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            {selectedEgg.variables.filter((variable) => variable.user_viewable !== false).map((variable) => {
              const isSecret = Boolean(variable.secret);
              const isVisible = visibleSecrets[variable.env];
              const inputType = isSecret && !isVisible ? "password" : variable.field_type === "number" ? "number" : "text";
              return (
                <div key={variable.env} className="space-y-2 rounded-lg border border-border bg-background/30 p-4">
                  <div className="flex items-center justify-between gap-2">
                    <Label htmlFor={variable.env} className="flex items-center gap-2">
                      {variable.name}
                      {variable.rules.includes("required") && <Badge variant="outline">required</Badge>}
                      {isSecret && <Badge variant="secondary" className="gap-1"><LockKeyhole className="h-3 w-3" />secret</Badge>}
                    </Label>
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      onChange={(e) => setVariables((current) => ({ ...current, [variable.env]: e.target.value }))}
                    ...
                    {isSecret && (
                      <Button
                        ...
                        variant="outline"
                        size="icon"
                        onClick={() => setVisibleSecrets((current) => ({ ...current, [variable.env]: !current[variable.env] }))}
