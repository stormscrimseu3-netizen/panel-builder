      ...
      <header className="mt-3 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground mono">
            {server.egg_name ?? server.runtime} · {server.memory_mb}MB · {server.cpu_percent}% CPU ·{" "}
            {server.node_id}
          </p>
        </div>
        <div className="flex gap-2">
          <Button
            size="sm"
            variant="outline"
            disabled={server.status === "online" || setStatus.isPending}
          ...
          <Button
            size="sm"
            variant="outline"
            disabled={server.status === "offline" || setStatus.isPending}
          ...
          <Button
            ...
            onClick={() => {
              setStatus.mutate("starting");
              setTimeout(() => setStatus.mutate("online"), 800);
            }}
          >
          ...
          <div className="grid gap-4 lg:grid-cols-[1fr_340px]">
                ...
                {Object.entries(eggVariables).map(([key, value]) => {
                  ...
                  return (
                    <div key={key} className="rounded-lg border border-border bg-background/40 p-3">
                      ...
                      <p className="mono mt-1 truncate text-sm">
                        {secret ? "••••••••••••" : String(value ?? "")}
                      </p>
                    </div>
                  );
            ...
            <div className="card-elevated rounded-2xl p-6">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Permanently delete this server and all its files.
              </p>
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      This action cannot be undone. All files and logs will be removed.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
  ...
  const send = async () => {
    ...
    await supabase.from("console_logs").insert([
      { server_id: serverId, user_id: userId, level: "command", message: `$ ${cmd}` },
      {
        ...
        user_id: userId,
        level: "info",
        message: `Executed: ${cmd.slice(0, 80)}`,
      },
    ]);
    setCmd("");
    qc.invalidateQueries({ queryKey: ["console", serverId] });
        ...
        <div className="h-80 overflow-auto rounded-md bg-background/80 p-3">
          {logs.length === 0 ? (
            <p className="text-muted-foreground">
              Console is empty. Start the server to see output.
            </p>
          ) : (
            logs.map((l) => (
      ...
      <aside className="border-r border-border bg-background/40 p-3">
        <div className="flex gap-2">
          <Input
            ...
            className="mono h-8"
          />
          <Button size="icon" className="h-8 w-8" onClick={create}>
        ...
        {file ? (
          <>
            ...
            <Textarea
              ...
              className="min-h-[360px] mono text-sm"
            />
          </>
        ) : (
